...
exports.updateProfile=async(req,res)=>{
    try {
        ...
        //find Profile
        const userDetails=await Thekedar.findById(id);
        if (!userDetails) {
...
exports.deleteAccount=async(req,res)=>{
    try {
        ...
        //delete user
        await Thekedar.findByIdAndDelete({_id:id});
        //return response
        return res.status(200).json({
            success:true,
...
exports.getAllThekedars=async(req,res)=>{
    try {
        //get id
        const id=req.user.id;
        //validation and get user Details
        const userDetails=await Thekedar.findById(id).populate("additionalDetails").exec();
...
exports.updateDisplayPicture = async (req, res) => {
    try {
      ...
      console.log(image)
      const updatedProfile = await Thekedar.findByIdAndUpdate(
        { _id: userId },
