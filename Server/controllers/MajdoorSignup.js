...
exports.sendOTP=async(req,res)=>{
    try {
        ...
        const checkUserPresent=await Majdoor.findOne({email});
...
exports.signup=async(req,res)=>{
   try {
     ...
     const existingUser=await Majdoor.findOne({email});
     if(existingUser){
         return res.status(400).json({
             success:false,
...
exports.login=async(req,res)=>{
    try {
        ...
        const user=await Majdoor.findOne({email}).populate("addtionalDetails");
        if(!user){
            return res.status(401).json({
                success:false,
...
exports.changePassword=async(req,res)=>{
   try {
     //get data from req body
     const userDetails=await Majdoor.findById(req.user.id);
     ...
     const updatedUserDetails=await Majdoor.findByIdAndUpdate(req.user.id,
